    ...
    <div class="table-responsive">
        ...
        <table v-show="cart.products && cart.products.length" class="table" width="100%" cellspacing="0">
          ...
          <tbody>
                <tr v-for="(row, index) in cart.products" v-bind:key="index" v-bind:class="{ error: row.error, errorstock: row.stock == 0 }">
                    ...
                    <td class="align-middle text-center">
                        <p class='product-name m-0'>
                            <a v-bind:href="'/producto/'+row.slug">
                              {{ row.name }} <span v-if="row.color"> - {{ jsUcfirst(row.color) }}</span>
                            </a>

                            <template v-if="row.wedding_list_id">
                                <br><a v-bind:href="'programa-de-regalos/lista?codigo='+row.wedding_list_id"><span class='badge badge-danger'>Lista: {{ row.wedding_list_id }}</span></a>
                                <br><span v-if="row.recargo" class='badge badge-info'> Este producto tiene un recargo por delivery </span>
                            </template>
                    ...
                    <td class="align-middle text-center">
                        ...
                        <span class='unit_price' v-if="row.dsct != 0"> <strike> S/ {{ formatUnitPrice(row.price) }} </strike></span><br>                        
                        <span class='unit_price' v-if="row.dsct ==0"> S/ {{ formatUnitPrice(row.price) }}</span>
                        <span class='unit_price' v-if="row.dsct != 0"  style="color:red"> - S/ {{ formatUnitPrice(row.dsct) }} </span><br>
                        <span class='unit_price' v-if="row.dsct != 0"  style="color:green"><strong>S/ {{ formatUnitPrice(row.price-row.dsct ) }} </strong></span>
                    </td>
                    <td class="align-middle text-center">
                        ...
                        <template>
                          <input v-on:keyup="updateQuantity(index, $event, row)" size="2" autocomplete="off" class="cart_quantity_input form-control" v-model="row.quantity" pattern='[0-9]*' type="text" min="1" :disabled="row.stock == 0">
                        </template>

                        <div class='cart_quantity_button clearfix'></div>
                    </td>
                    <td class="align-middle text-center">
                        <span v-on:click="removeElement(index, row, $event )" class="cart_quantity_delete"> <i class='fas fa-trash-alt'></i></span>
                    </td>
                    <td class="align-middle text-center">
          ...
          <tfoot>
              <tr>
                <td colspan="3"></td>
                <td colspan="2" class="total_price_container text-right"> <b>Subtotal</b></td>
                <td colspan="1" class="price align-middle text-center" id="total_price_container">
                  <span id="total_price">S/ {{ getSubTotal() }}</span>
                </td>
              </tr>
              <tr>
                <td colspan="3"></td>
                <td colspan="2" class="total_price_container text-right"> <b>Costo de envío</b></td>
                <td colspan="1" class="price align-middle text-center" id="total_price_container">
              ...
              <tr class="cart_total_price">
                <td colspan="3"></td>
                <td colspan="2" class="total_price_container text-right"> <b>Total</b></td>
                <td colspan="1" class="price align-middle text-center" id="total_price_container">
                  <span id="total_price">S/ {{ getTotal() }}</span>
                </td>
              </tr>
          </tfoot>
        </table>
